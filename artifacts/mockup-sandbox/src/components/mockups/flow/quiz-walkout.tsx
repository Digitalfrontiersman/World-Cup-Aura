...
const WALKOUT_OPTIONS = [
  "Final boss energy",
  "Locked in and silent",
  "Pure national pride",
  "Main character mode",
...
export default function QuizWalkoutScreen() {
  return (
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", gap: 20, flex: 1 }}>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {Array.from({ length: 7 }, (_, i) => (
            <div key={i} style={{
              flex: 1, height: i === 6 ? 8 : 6, borderRadius: 999,
              background: i === 6 ? "#fbbf24" : i < 6 ? "rgba(251,191,36,0.5)" : "#374151",
              boxShadow: i === 6 ? "0 0 8px rgba(251,191,36,0.7)" : "none",
            }} />
          ))}
        </div>
        ...
        <h2 style={{ color: "#fff", fontSize: 30, fontWeight: 700, margin: 0, lineHeight: 1.2 }}>
          Choose your walkout vibe
        </h2>

        {/* Tap any option to advance to scanner */}
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", gap: 10 }}>
          {WALKOUT_OPTIONS.map((opt, i) => (
            <button
              key={opt}
