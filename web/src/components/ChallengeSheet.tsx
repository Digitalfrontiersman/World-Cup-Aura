...
export function ChallengeSheet({ open, onClose, cardDataUrl, rarity, archetype, rank, shareUrl: shareUrlProp, mintExplorerUrl }: ChallengeSheetProps) {
  ...
  const shareMessage = `I just got my Aura Card - ${rarity} ${archetype} with ${rank} rank. Bet yours can't top it. Get yours: ${shareUrl}`;
  ...
  const handleNativeShare = async () => {
    ...
    try {
      ...
      await navigator.share({ title: "My Aura Card", text: shareMessage, url: shareUrl });
    } catch {
      /* user dismissed */
    }
  };
